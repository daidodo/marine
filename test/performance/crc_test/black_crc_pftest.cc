...
static inline unsigned int Crc(unsigned int init, const char * buf, size_t sz)
{
    const unsigned int kCodes[256] = {
        0x0, 0xB71DC104, 0x6E3B8209, 0xD926430D, 0xDC760413, 0x6B6BC517, 0xB24D861A, 0x550471E, 0xB8ED0826, 0xFF0C922,
        ...
        0xDF2B32A6, 0x6836F3A2, 0x6D66B4BC, 0xDA7B75B8, 0x35D36B5, 0xB440F7B1
    };
    unsigned int        result;
    const unsigned int        *p = (const unsigned int *)buf;
    const unsigned int        *e = (const unsigned int *)(buf + sz);
    if(sz < 4)abort();
    result = ~*p++;
    while( p<e )
    {
#if defined(LITTLE_ENDIAN)
        result = kCodes[result & 0xff] ^ result >> 8;
        result = kCodes[result & 0xff] ^ result >> 8;
        result = kCodes[result & 0xff] ^ result >> 8;
        result = kCodes[result & 0xff] ^ result >> 8;
        result ^= *p++;
#else
        result = kCodes[result >> 24] ^ result << 8;
        result = kCodes[result >> 24] ^ result << 8;
        result = kCodes[result >> 24] ^ result << 8;
        result = kCodes[result >> 24] ^ result << 8;
        result ^= *p++;
