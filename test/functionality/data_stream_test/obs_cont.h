...
{
    {
        ...
        __OutStream out(100, true);
        ASSERT_EQ(0, out.status());
        ASSERT_EQ(le(true), out.littleEndian());
    }{
        __OutStream out(100, false);
        ASSERT_EQ(0, out.status());
...
TEST(__OutStream, finish)
{
    ...
    const char v[] = ";n32kq-b q3 -9qef q;fdkf";
    {
        ...
        ASSERT_TRUE(ds.finish(x));
        ASSERT_EQ(size_t(0), ds.cur());
        ASSERT_EQ(sizeof v, x.size());
        ASSERT_EQ(0, memcmp(v, &x[0], sizeof v));
    }{
        __Buff x(p, p + sizeof p);
        __OutStream ds;
        ASSERT_EQ(0, ds.status());
        ...
        ASSERT_TRUE(ds.finish(x));
        ASSERT_EQ(size_t(0), ds.cur());
        ASSERT_EQ(sizeof p + sizeof v, x.size());
        ASSERT_EQ(0, memcmp(p, &x[0], sizeof p))<<"x="<<tools::Dump(x);
        ASSERT_EQ(0, memcmp(v, &x[sizeof p], sizeof v));
    }{
        __Buff x(4096, 0);
        __OutStream ds;
        ASSERT_EQ(0, ds.status());
        ...
        ASSERT_EQ(size_t(0), ds.cur());
        ASSERT_EQ(sizeof p + sizeof v, x.size());
        ASSERT_EQ(0, memcmp(p, &x[0], sizeof p));
        ASSERT_EQ(0, memcmp(v, &x[sizeof p], sizeof v));
    }{
        __Buff x(4096, 0);
        __OutStream ds;
        ...
        size_t sz = x.size();
        ASSERT_EQ(&ds, &(ds<<Manip::end(&x[0], &sz)));
        ASSERT_EQ(size_t(0), ds.cur());
        ASSERT_EQ(sizeof v, sz);
        ASSERT_EQ(0, memcmp(v, &x[0], sizeof v));
    }
