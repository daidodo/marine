#include <marine/posix_shm_file.hh>

#include "../inc.h"
#include <marine/tools/debug.hh>

TEST(CPosixShmFile, all)
{
    ...
    CPosixShmFile f2("not_exist_file");
    ASSERT_FALSE(f2.valid());

    CPosixShmFile f3(FNAME, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);

    //valid
    ASSERT_TRUE(f3.valid());

    //fd
    ASSERT_TRUE(CPosixShmFile::kInvalidFd == f1.fd());
    ASSERT_TRUE(CPosixShmFile::kInvalidFd == f2.fd());
    ASSERT_TRUE(CPosixShmFile::kInvalidFd != f3.fd());

    //fdType
    ASSERT_TRUE(CPosixShmFile::kFdType == f1.fdType());
    ASSERT_TRUE(CPosixShmFile::kFdType == f2.fdType());
    ASSERT_TRUE(CPosixShmFile::kFdType == f3.fdType());

    //fdTypeName
    ASSERT_STREQ("CPosixShmFile", f1.fdTypeName());
    ASSERT_STREQ("CPosixShmFile", f2.fdTypeName());
    ASSERT_STREQ("CPosixShmFile", f3.fdTypeName());

    //pathname
    ASSERT_NE(std::string(), f3.filename());

    //shmname
    ASSERT_EQ(FNAME, f3.shmname());
    ...
    //truncate
    ASSERT_TRUE(f3.valid());
    ASSERT_TRUE(f3.truncate(1000))<<f2.toString()<<endl<<CPosixShmFile::ErrMsg();
#endif
    ...
    oss<<"{CFile={IFileDesc={fd_="<<f3.fd()<<"(CPosixShmFile)}, filename="<<f3.filename()<<"}, shmname_="<<f3.shmname()<<"}";
    ASSERT_EQ(oss.str(), f3.toString())<<"f3="<<f3.toString()<<endl;

    //rename
    std::string fname = f3.filename();
    ASSERT_TRUE(f3.rename("/dev/shm/rn.txt"))<<CPosixShmFile::ErrMsg();
    ASSERT_TRUE(CPosixShmFile::Rename("/dev/shm/rn.txt", fname))<<CPosixShmFile::ErrMsg();

    //unlink
    ASSERT_TRUE(CPosixShmFile::Unlink(FNAME));
}
